...
impl LogicalMeterHandle {
    ...
    /// Asks the graph for the formula selected by `M::KIND` and `request`,
    /// parses it, and tags every component leaf with `M::METRIC`.
    fn formula<M: Metric>(&self, request: GraphRequest) -> Result<Formula<M::QuantityType>, Error> {
        let name = request.name();
        let graph = &self.graph;
        let generated = match (M::KIND, request) {
...
fn tag_components(
    ...
    metric: MetricPb,
) -> Result<crate::FormulaExpr, Error> {
    Ok(generated
        .to_string()
...
mod tests {
    ...
    async fn test_pv_reactive_power_formula() {
        ...
        check_samples(
            ...
            TimeDelta::try_seconds(1).unwrap(),
            // The PV inverter is the coalesce's primary and reports no
            // reactive power, so the first tick is spent learning that and
            // subscribing to the PV meter fallback: it reads `None`.
            vec![
                None,
                Some(-1.4),
                Some(-0.5),
    ...
    async fn test_battery_voltage_formula() {
        ...
        check_samples(
            ...
            // voltage, so the first tick is spent subscribing to the
            // inverter fallback: it reads `None`.
            vec![
                None,
                Some(398.0),
                Some(397.67),
    ...
    async fn test_resampling_functions() {
        ...
        check_samples(
            ...
            // As in `test_battery_voltage_formula`, the first tick is the
            // one that pulls the inverter fallback into the subscription
            // set, so it reads `None`.
            vec![
                None,
                Some(400.0),
                Some(400.0),
    ...
    async fn test_mixed_metric_formula() {
        ...
        for i in 0..4 {
            match (base[i].value(), averaged[i].value()) {
                // The first tick is the one that pulls the coalesce's
                // fallback into the subscription set, so both formulas,
                // which share the same components, read `None` for it.
                (None, None) => {}
                (Some(b), Some(a)) => {
